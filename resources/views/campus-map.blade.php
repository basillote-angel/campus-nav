            ...
            <div class="py-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="mb-4 sm:mb-0">
                        ...
                        <h1 class="text-3xl font-bold text-[#123A7D]">Campus Navigation</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="bg-gray-100 rounded-lg p-1 flex space-x-1">
                        {{-- Map View is active by default with new color --}}
                        <button id="mapViewBtn" class="view-toggle px-4 py-2 text-sm font-semibold rounded-md transition-colors duration-200 
                                text-white shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2" 
                                style="background-color: #123A7D; hover:background-color: #0E2F5E; focus:ring-color: #123A7D;">
                        ...
                        {{-- List View is inactive by default --}}
                        <button id="listViewBtn" class="view-toggle px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 
                                text-gray-700 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            <svg class="w-4 h-4 mr-2 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div id="mapSection" class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h3 class="text-lg font-medium text-gray-900 flex items-center">
                    {{-- Updated: Map icon uses the new color via style --}}
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color: #123A7D;">
        ...
        <div id="buildingListSection" class="hidden">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-medium text-gray-900 flex items-center">
                        {{-- Updated: List icon uses the new color via style --}}
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="color: #123A7D;">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    {{-- Updated: Category badge uses the new color's lighter shade --}}
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" style="background-color: #D3E0E8; color: #123A7D;">
...
<script>
    ...
    // Define the new color variables for use in JavaScript
    const PRIMARY_COLOR = '#123A7D';
    const PRIMARY_HOVER_COLOR = '#0E2F5E'; // A slightly darker shade for hover/active

    /**
     * Toggles the active/inactive visual state of the view buttons.
     */
    function activateButton(activeBtn, inactiveBtn) {
        // Set active button (new color background)
        activeBtn.style.backgroundColor = PRIMARY_COLOR;
        activeBtn.classList.add('text-white', 'shadow-md', 'font-semibold');
        activeBtn.classList.remove('text-gray-700', 'hover:bg-gray-200', 'font-medium');
        
        // Custom hover/focus styles for the active button
        activeBtn.onmouseover = () => activeBtn.style.backgroundColor = PRIMARY_HOVER_COLOR;
        activeBtn.onmouseout = () => activeBtn.style.backgroundColor = PRIMARY_COLOR;

        // Set inactive button (gray text, hover background)
        inactiveBtn.style.backgroundColor = '';
        inactiveBtn.classList.remove('text-white', 'shadow-md', 'font-semibold');
        inactiveBtn.classList.add('text-gray-700', 'hover:bg-gray-200', 'font-medium');
        
        // Remove custom hover/focus styles from the inactive button
        inactiveBtn.onmouseover = null;
        inactiveBtn.onmouseout = null;
    }
    ...
    mapBtn.addEventListener('click', () => {
        ...
        // Reinitialize map if it was destroyed or container size changed
        if (map) {
            setTimeout(() => { map.invalidateSize(); }, 0); // Important for Leaflet
        } else {
            initializeMap();
        }
    });
    ...
    function initializeMap() {
        ...
        const buildings = @json($buildings ?? []); // Use the $buildings variable if available, otherwise an empty array
    ...
    document.addEventListener('DOMContentLoaded', function() {
        initializeMap();
        // Ensure the map button is visually active on load
        activateButton(mapBtn, listBtn); 
    });
